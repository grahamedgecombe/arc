...
static mpfp_t *mpfp_search_range(uintptr_t start, uintptr_t end)
{
...
mpfp_t *mpfp_search(void)
{
  ...
  uintptr_t ebda = (*((uint16_t *) aphy32_to_virt(BDA_EBDA))) << 4;
 
  /* search the first kilobyte of the EBDA */
  mpfp_t *mpfp = mpfp_search_range(ebda, ebda + 1024);
  if (mpfp)
    return mpfp;

  /* search between 639K and 640K */
  mpfp = mpfp_search_range(0x9FC00, 0xA0000);
  if (mpfp)
    return mpfp;

  /* search between 511K and 512K */
  mpfp = mpfp_search_range(0x7FC00, 0x80000);
  if (mpfp)
    return mpfp;

  /* search the shadow BIOS ROM */
  mpfp = mpfp_search_range(0xF0000, 0x100000);
  if (mpfp)
    return mpfp;
